...
unidadesTotalesPorContinentes(Jugador, UnidadesTotales):-
    ...
    findall(
        Cantidad, 
        unidadesQueLeTocanPorContinenteCompleto(Jugador, _, Cantidad),
        Cantidades
    ),
...
unidadesCorrespondientesPorPaises(Jugador, Unidades):-
    ...
    unidadesPorPais(CantidadPaises, Unidades).

unidadesPorPais(Paises, Unidades):-
    Unidades is Paises // 2,
    Paises > 6.
unidadesPorPais(Paises, 3):- Paises =< 6.
